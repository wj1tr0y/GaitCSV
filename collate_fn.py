...
class CollateFn(object):
    def __init__(self, label_set, view_set, sample_config):
        ...
        if self.sampler not in ['fixed', 'unfixed', 'all']:
            raise ValueError("The first item in sampler_type  must be in ['fixed', 'unfixed', 'all'].")
        if self.ordered not in ['ordered', 'unordered']:
            raise ValueError("The second item in sampler_type must be in ['ordered', 'unordered'].")
    ...
    def __call__(self, batch):
        ...
        for bt in batch:
            ...
            ############# view to tensor ###############
            vies_batch.append(self.view_set.index(bt[1][2]))
            ################################################
