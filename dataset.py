...
class DataSet(tordata.Dataset):
    def __init__(self, data_cfg, training):
        ...
        self.view_dict = {label: [] for label in self.label_set}

        for i, seq_info in enumerate(self.seqs_info):
            self.indices_dict[seq_info[0]].append(i)

        ############ ids for each view ################
        self.indices_dict_view_order = {view: {label:[] for label in self.label_set} for view in self.views_set}
        for i, seq_info in enumerate(self.seqs_info):
            self.indices_dict_view_order[seq_info[2]][seq_info[0]].append(i)

        emptys = []
        for v in list(self.indices_dict_view_order.keys()):
            for l in list(self.indices_dict_view_order[v].keys()):
                if len(self.indices_dict_view_order[v][l]) == 0:
                    emptys.append((v, l))
        ###############################################
        for v, l in emptys:
            del self.indices_dict_view_order[v][l]
    ...
    def __loader__(self, paths):
        ...
        for pth in paths:
            if pth.endswith('.pkl'):
                ...
                raise ValueError('- Loader - just supports .pkl !!! \
                                    Please use scripts in data folder to preprocess dataset.')
            data_list.append(_)
